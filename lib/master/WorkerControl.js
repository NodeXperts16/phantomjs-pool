...
Worker.prototype.createProcess = function() {
    ...
    var clArgs = [__dirname + '/../../lib/worker/Worker.js', this.id, this.pool.workerFile];
    ...
    this.proc.stderr.on('data', function (data) {
        data.toString().split('\n').forEach(function(line) {
            if (line.trim().length !== 0) {
                console.log('  #' + that.id + ' STDERR >> ' + line); // TODO: Write this into STDERR
            }
        });
    });
    ...
    this.proc.on('close', function (code, signal) {
        ...
        if (signal !== 'SIGTERM') {
            ...
            if (code !== 0) { // sigterm means we killed the worker on our own
                ...
                if (that.currentJob && that.currentJob.callback) {
